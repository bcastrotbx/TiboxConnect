...
- [Fase 10 — Analítica de comportamiento (Fase 1: base + vistas iniciales)](phases/FASE-10-ANALITICA-FASE1.md) — tabla `analytics_events` + RLS con `is_admin()`, `trackPageView` instrumentado en `PortalLayout` (excluye `/admin`), y nueva página `/admin/analitica` con "Resumen general" y "Secciones más visitadas" (primera dependencia de UI del proyecto: `recharts`). Tracking de video/CTAs/formularios y Microsoft Clarity quedan para fases siguientes. **La migración debe ejecutarse antes de que la analítica registre datos.**
