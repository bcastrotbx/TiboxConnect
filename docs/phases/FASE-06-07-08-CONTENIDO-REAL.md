...
- **Ya ejecutadas y confirmadas** (se dejan de listar como pendientes): `20260729100000_webinars_category.sql`, `20260729100100_hero_slides_seed.sql`, `20260729100200_storage_content_images.sql`, `20260730100000_demo_content_agosto.sql`, `20260731100000_events_sort_order.sql`, `20260731100100_fix_infographic_thumbnail_duplicates.sql`, `20260731100200_site_settings.sql`, `20260731100300_services.sql`, `20260731100400_grants_site_settings_services.sql`, `20260801100000_events_partner_logo.sql` (confirmado en vivo contra la base real antes del commit/push de este ajuste: `GET /rest/v1/events?select=partner_logo_url` responde `200` con la columna presente). **No hay ninguna migración SQL nueva ni pendiente en este ajuste** (fix de `useDragScroll.js`, página de detalle de Eventos y navegación de `EventosPage.jsx` — todo cambios de código, ninguno de esquema).
...
## Ajuste posterior — fix de clicks suprimidos por drag-to-scroll, página propia de detalle de Eventos, popup de Infografías en el home

Pedido de Braulio: tres bugs/mejoras de comportamiento en Videos, Eventos e Infografías, todos revisados en local antes de aprobar.

- **Bug raíz — "Ver video"/"Ver infografía"/"Ver detalles" dejaron de abrir su popup en el home.** Primer diagnóstico (incompleto): se asumió que `DRAG_THRESHOLD` (6px) del drag-to-scroll agregado en el ajuste anterior era demasiado sensible, y se subió a 18px — un cambio real pero insuficiente, como confirmó Braulio al seguir viendo el bug en local después de ese primer intento. Diagnóstico correcto, encontrado instrumentando el hook con `console.log` y probando con un clic real de mouse (vía automatización de navegador con eventos de puntero nativos — un `.click()` sintético o un `dispatchEvent` manual **no** reproducía el bug, lo que explica por qué el primer intento pareció verificado): `onPointerDown` llamaba a `el.setPointerCapture(e.pointerId)` en **todo** clic, no solo al confirmarse un arrastre. Con el puntero capturado desde el primer instante, Chrome retargetea el evento `click` posterior al elemento que capturó el puntero (el track del carrusel) en vez del elemento real bajo el cursor (el botón o la tarjeta) — confirmado con logging: el `target` del clic terminaba siendo el track completo. El botón nunca llegaba a recibir su propio evento `click` (no está en el trayecto de despacho cuando el `target` es un ancestro suyo), así que su `onClick` no se disparaba nunca, sin importar cuánto se hubiera movido el mouse ni el valor de `DRAG_THRESHOLD`. **Corrección real:** `el.setPointerCapture(...)` se movió de `onPointerDown` a `onPointerMove`, y solo se llama la primera vez que el movimiento supera `DRAG_THRESHOLD` (`src/hooks/useDragScroll.js`) — un clic simple, sin ese movimiento, nunca captura el puntero y conserva su `target` real. Verificado con clics reales (no sintéticos) en las tres secciones del home que el popup abre correctamente, y con un arrastre real (drag de ~200px) que el carrusel se sigue desplazando y su clic de cierre se sigue suprimiendo. Este único fix resolvió los tres síntomas reportados (Videos, Eventos e Infografías comparten el mismo hook) — el popup de Infografías del home ya usaba el mismo componente `InfografiaModal` que la página `/infografias` (formulario de lead + botón Descargar), no había ninguna inconsistencia de componente que corregir aparte de esto.
...
## Ajuste posterior — formulario de eventos: quitar hora/fecha de término, simplificar Estado, galería de fotos

Pedido de Braulio: cambios puntuales al formulario "Nuevo evento / Editar evento" del admin. Antes de tocar los campos con riesgo de romper otras partes del sitio (fecha/hora de término, Estado), se hizo un análisis de impacto y se propuso una solución, confirmada por Braulio antes de implementar.

- **Hallazgo clave del análisis:** la política de seguridad (RLS) de `events` (`events_select_public`, Fase 4) **solo permite al público leer eventos con `status='published'`** — nunca `status='completed'`. Es decir, el estado "Completado" que existía en el formulario nunca funcionó de verdad en producción: cualquier evento marcado así desaparecía por completo del sitio público (RLS lo bloqueaba antes de que el código pudiera mostrarlo, tanto en "próximos" como en "realizados"). Confirmado contra los 3 eventos reales existentes: los 3 tienen fecha ya pasada pero siguen con `status='published'`, y por lo tanto no aparecían como "realizados" bajo la lógica anterior (`status='completed'` requerido) — quedaban invisibles en ambas listas.
- **Solución implementada:** "¿ya se realizó?" ahora se calcula comparando `starts_at` con la hora actual (`eventService.isEventPast()`, nuevo, exportado para reutilizar en `Events.jsx`/`EventoDetailPage.jsx`/`AdminWidgets.jsx`), en vez de depender de un estado manual. Esto corrige de paso el bug de RLS descrito arriba — confirmado en vivo: tras el cambio, los 3 eventos reales pasaron a mostrarse correctamente como "realizados" (sin badge "PRÓXIMAMENTE"), y la página `/eventos/evento-de-prueba-xv5acm` mostró "EVENTO REALIZADO" correctamente.
- **`src/services/eventService.js`:** `getPastEvents()`, `getEventBySlug()`, `getEventDetailBySlug()`, `getEventById()` — todos los filtros por `status='completed'`/`in(['published','completed'])` pasaron a `status='published'` (+ comparación de fecha donde corresponde), alineado con lo único que RLS permite.
- **Campo Estado del formulario de eventos** reducido a Borrador/Publicado únicamente (antes tenía también Completado/Archivado). Se quitaron del menú de acciones de cada fila del admin: "Marcar como realizado" (ya no aplica, es automático) y "Archivar" para eventos específicamente (para no dejar un evento en un estado que el formulario ya ni ofrece) — ambas acciones se mantienen sin cambios para Videos/Infografías/Noticias.
- **Campos "Fecha de término"/"Hora de término" eliminados del formulario.** La columna `ends_at` de la base de datos **no se borró** (decisión deliberada, más segura y reversible) — sigue existiendo sin usarse desde el formulario; eventos ya guardados con hora de término la conservan y la siguen mostrando (`formatTimeRange` ya toleraba que faltara). `isReorderable` (drag-and-drop del admin) pasó de mirar `status==='completed'` a la misma comparación de fecha.
- **Placeholder de "Lugar"** cambiado a "Curicó, Santiago…".
- **Galería de fotos del evento** (hasta 8 imágenes): nueva migración `20260810100000_events_gallery.sql` (columna `gallery text[]`, con `check` de máximo 8 — la tabla nunca había tenido esta columna; `event.gallery` ya se leía en el frontend pero siempre caía en el fallback vacío porque el dato no existía en la base real). Nuevo componente `GalleryUploadField` en `AdminWidgets.jsx`: sube a Storage (bucket `content-images`, mismo `storageService.uploadContentImage` que el resto de las imágenes del admin, sin cambios), vista previa en grilla, botón de eliminar individual por foto — que borra el archivo de Storage de inmediato, no solo al guardar. `adminEventsService.updateEvent`/`deleteEvent` limpian en Storage las fotos que salen de la galería (comparando contra la galería anterior) o que pertenecían a un evento eliminado, mismo criterio que ya existía para banner/logo del colaborador.
- **Sin migración para `ends_at`** (se deja la columna, ver arriba) — solo la migración nueva de `gallery`.
- **Verificado en este entorno de trabajo:** se montó temporalmente una ruta de prueba sin autenticación para el formulario (mismo patrón ya usado en ajustes anteriores, revertida por completo — `git status`/`git diff` confirmaron cero residuos) — se confirmó visualmente el placeholder de Lugar, la ausencia de los campos de término, la sección de galería con el contador "(0/8)" y el límite dinámico, y que Estado solo ofrece Borrador/Publicado. Se verificó en vivo, contra datos reales, que `/eventos`, el detalle de evento y el home siguen funcionando sin errores de consola tras el cambio de lógica próximos/realizados (incluida la corrección del bug de RLS descrito arriba). `npm run lint` y `npm run build` limpios. La subida real de archivos a la galería no se probó con un archivo real en este entorno (reutiliza `uploadContentImage`, ya validado en sesiones anteriores para banner/logo/infografías) — y el guardado de un evento con fotos de galería no puede probarse de punta a punta hasta que Braulio ejecute la migración nueva en Supabase.

**Confirmado por Braulio, commiteado y pusheado a `origin/main`** (commit `28abeae`) — la migración `20260810100000_events_gallery.sql` seguía sin ejecutarse al momento del push (verificado en vivo: `column events.gallery does not exist`); el resto del código funciona igual sin ella, solo la galería de fotos queda pendiente de esa migración.

## Ajuste posterior — altura pareja en las tarjetas de evento (home y /eventos)

Pedido de Braulio: las tarjetas de `EventCard` (usadas en el panel de Eventos del home y en la página `/eventos`) quedaban con distinta altura según el largo del título y, sobre todo, del resumen — un evento sin resumen (ej. "evento de prueba") quedaba con una tarjeta visiblemente más baja que sus vecinas en la misma fila.

- **Causa:** el párrafo del resumen ya usaba recorte a 2 líneas (`WebkitLineClamp:2`) para textos largos, pero no reservaba una altura mínima — con texto corto o vacío, el párrafo colapsaba a su alto natural (casi cero si está vacío) en vez de ocupar el espacio de 2 líneas completas, empujando hacia arriba el resto del contenido (colaborador, botón "Ver detalles"). El título tenía el mismo problema en sentido inverso: sin límite de líneas, un título largo podía crecer a 3+ líneas y estirar la tarjeta más que sus vecinas.
- **Fix (`src/components/Events.jsx`, `EventCard`):** mismo patrón ya usado en `VideoCard`/`InfoCard` (`Media.jsx`) — `minHeight` reservado en `em` (según `fontSize`/`lineHeight` de cada elemento) combinado con el recorte a 2 líneas ya existente: título `minHeight:'2.6em'` (agregado el recorte de 2 líneas, que no tenía), resumen `minHeight:'3em'` (ya tenía el recorte, solo le faltaba el mínimo). Con esto, el bloque de texto ocupa siempre el mismo alto sin importar si el contenido está vacío, es corto o es largo — no fue necesario fijar una altura total a la tarjeta completa (que además habría sido problemático dado que la imagen usa `aspect-ratio` y varía de alto según el ancho de columna, distinto entre el carrusel del home y la grilla de 4 columnas de `/eventos`).
- **Verificado en el navegador**, en `/eventos` y en el panel de Eventos del home, con tres escenarios sobre las mismas tarjetas reales: resumen normal (2 líneas), resumen de una sola línea ("evento de prueba"/"prueba") y, inyectando contenido de prueba vía consola sobre el DOM ya renderizado (sin tocar datos reales), un resumen muy largo y un resumen vacío — en los tres casos las tarjetas de la misma fila quedaron con altura idéntica, el texto largo se recortó con "…" sin desbordar, y el botón "Ver detalles"/colaborador se mantuvieron en la misma posición vertical en todas. `npm run lint` y `npm run build` limpios.
- **Sin migración SQL** — cambio puramente visual (CSS inline), no toca datos.

**Commit y push directo a `origin/main`, sin revisión previa en local** — a pedido explícito de Braulio para este ajuste puntual.

## Ajuste posterior — galería de fotos de eventos: de subida a Supabase Storage a enlaces (links)

Pedido de Braulio: la galería de fotos del evento (agregada en el ajuste anterior) pasó de subir archivos a Supabase Storage a que el admin pegue enlaces de imágenes ya alojadas en otro lugar (ej. WordPress) — para no seguir gastando espacio de Storage en fotos de eventos. Braulio reportó además el error real que motivó revisar el estado de la migración: `"Could not find the 'gallery' column of 'events' in the schema cache"`.

- **Diagnóstico del error reportado:** no era un problema de nombre/tipo de columna distinto ni de schema cache desincronizado — la columna `gallery` **nunca se había creado** en el proyecto real (confirmado en vivo, otra vez: `column events.gallery does not exist`). La migración `20260810100000_events_gallery.sql` seguía sin ejecutarse desde que se escribió. El mensaje de error de PostgREST es el mismo tanto si la columna no existe como si existe pero el schema cache de PostgREST está desactualizado — se agregó de todas formas `notify pgrst, 'reload schema';` al final de la migración, para cubrir ambos casos de una vez (si alguna vez SÍ llega a estar desincronizado el cache, este comando lo fuerza a recargar sin esperar un restart del proyecto).
- **Migración reescrita, no una migración de corrección aparte** (`supabase/migrations/20260810100000_events_gallery.sql`) — como nunca se había ejecutado contra el proyecto real, se editó directo en vez de agregar una segunda migración. Cambios: límite subido de 8 a 10 (pedido explícito de este ajuste) y ya no se menciona Storage en los comentarios — sigue siendo `text[]`, el tipo de columna no cambió, solo lo que contiene (URLs externas en vez de URLs de nuestro propio bucket).
- **Formulario (`src/admin/AdminWidgets.jsx`):** el bloque "Arrastra fotos o haz clic para subir" (`GalleryUploadField`) se reemplazó por `GalleryLinksField` — una fila por enlace, en grilla de 2 columnas, cada una con su botón de eliminar (✕) y un botón general "Agregar enlace" (deshabilitado/oculto al llegar a 10). Cada campo es un `<input type="url">` (bloqueo nativo del navegador si el formato no es una URL válida, antes de que el formulario intente enviarse) más una validación propia con `isValidHttpUrl()` (mensaje en rojo bajo el campo) y una advertencia no bloqueante si la URL no termina en una extensión de imagen conocida (`.jpg/.png/.webp/.gif/.avif/.svg`) — puede ser una URL sin extensión que igual sirva una imagen, así que no se bloquea el guardado por eso, solo por URLs realmente mal formadas. Al guardar, `handleSubmit` limpia filas vacías y bloquea el envío (con el mismo banner de error que ya usa el resto del formulario) si queda alguna URL inválida.
- **Limpieza de Storage retirada para la galería** (`src/services/adminEventsService.js`, `src/services/storageService.js`): como las fotos ya no viven en nuestro bucket, se quitó la lógica que borraba archivos de `content-images` al editar/eliminar un evento con galería (`deleteContentImageUnconditional`, ahora eliminada por completo — no le quedaba ningún otro uso). El banner/logo del colaborador siguen limpiándose igual que antes, sin cambios — solo la galería dejó de tocar Storage.
- **Visualización pública sin cambios** (`VistaModal` en `Events.jsx`, `EventoDetailPage.jsx`): ya renderizaba cada foto con `<img src={src}>` genérico, sin ninguna dependencia de que la URL fuera de Supabase Storage — funciona igual con URLs externas, no hizo falta tocar nada ahí.
- **Verificado en el navegador**, con dos rutas de prueba temporales sin autenticación (mismo patrón ya usado en ajustes anteriores, revertidas por completo — `git status`/`git diff` confirmaron cero residuos):
  - **Formulario:** una URL mal formada quedó bloqueada por la validación nativa del navegador (no se pudo enviar el formulario) y mostró el mensaje en rojo bajo el campo; corregida la URL, "Agregar enlace" sumó una segunda fila en la grilla de 2 columnas; se agregaron enlaces hasta el máximo de 10 (confirmado: el botón "Agregar enlace" desaparece exactamente al llegar a 10, no antes ni después) y el contador de la etiqueta reflejó correctamente cuántos campos estaban realmente llenos (no cuenta filas vacías); eliminar un enlace con el botón ✕ quitó la fila y volvió a mostrar "Agregar enlace".
  - **Visualización pública:** con datos de prueba (dos imágenes reales por URL externa + una URL rota a propósito), `VistaModal` mostró las dos imágenes correctamente cargadas y la rota degradó a un ícono de imagen roto sin romper el resto de la página ni tirar errores de consola.
  - `npm run lint` y `npm run build` limpios.
- **Migración pendiente de que Braulio la ejecute** (sigue igual que antes de este ajuste, solo cambió el contenido del archivo — ver arriba): `20260810100000_events_gallery.sql`, en el SQL Editor de Supabase.
...
## Ajuste posterior — subida directa de imágenes de galería a WordPress (endpoint propio + clave secreta)

Pedido de Braulio: además de pegar enlaces a mano, agregar un botón "Subir imagen" en la galería de fotos del evento que suba el archivo directo a la Biblioteca de Medios de WordPress (tibox.cl). Se descartó Application Passwords de WordPress (problemas para activarlas en el sitio) — la alternativa es un endpoint REST propio en WordPress, autenticado con una clave secreta fija (no ligada a ningún usuario), agregado vía el plugin Code Snippets.

Tres piezas nuevas, documentadas en detalle en `docs/integrations/wordpress-image-upload.md`:

1. **`docs/integrations/wordpress-upload-image-snippet.php`** — el código exacto para que Braulio pegue en Code Snippets (WordPress). Registra `POST /wp-json/tibox/v1/upload-image`, valida el header `X-Tibox-Secret` contra una clave fija definida en el propio snippet (`hash_equals`, no una comparación directa — evita timing attacks), y si coincide sube con `media_handle_upload()` (cargando a mano `wp-admin/includes/{image,file,media}.php`, que no se cargan solos en el contexto de la REST API) y devuelve `{ "url": ... }`. `permission_callback` es `__return_true` a propósito — la validación real es la clave secreta dentro del callback, no un usuario de WordPress.
2. **`api/upload-event-image.js`** (nueva, Vercel Serverless Function — primera de este tipo en el proyecto) — recibe el archivo en crudo del frontend (no multipart, para no depender de una librería de parseo), arma el `multipart/form-data` que WordPress espera con las APIs nativas de Node 18+ (`fetch`/`FormData`/`Blob`, sin dependencias nuevas) y agrega la clave secreta leída de `process.env.WP_UPLOAD_SECRET` — la única variable de entorno de esta pieza, nunca expuesta al navegador. Se agregó `"engines": {"node": ">=18.0.0"}` a `package.json` (esas APIs son globales recién desde Node 18) y un bloque nuevo en `eslint.config.js` para que `api/**/*.js` use `globals.node` en vez de `globals.browser` (si no, ESLint marca `Buffer`/`process` como no definidos).
3. **Frontend** (`src/services/wordpressUploadService.js`, nuevo; `GalleryLinksField` en `src/admin/AdminWidgets.jsx`) — botón "Subir imagen" junto a "Agregar enlace", valida tipo (JPG/PNG/WEBP) y tamaño (máx. 8MB) antes de subir, agrega la URL resultante como una fila más del mismo arreglo de enlaces (respeta el máximo de 10 combinando ambos orígenes).

- **Verificado en este entorno de trabajo** (no se puede probar de punta a punta hasta que Braulio pegue el snippet y configure `WP_UPLOAD_SECRET` en Vercel): con la misma ruta de prueba temporal ya usada en ajustes anteriores (revertida por completo), se confirmó que el botón "Subir imagen" aparece junto a "Agregar enlace"; que un archivo con tipo no permitido (PDF) y uno que excede 8MB se rechazan del lado del cliente con el mensaje correspondiente, sin llegar a intentar la subida; que un intento real contra `/api/upload-event-image` (inexistente en el servidor de desarrollo local) muestra el mensaje de error genérico sin romper el formulario; y que, interceptando `fetch` para simular una respuesta exitosa del endpoint, la URL devuelta se agrega correctamente como fila nueva y el contador se actualiza. `npm run lint` y `npm run build` limpios.
- **Sin migración SQL** — la galería sigue siendo `events.gallery` (`text[]`), sin cambios de schema; esta pieza solo agrega una forma más de llenarla.

**Sin commit ni push todavía** — a la espera de que Braulio pegue el snippet en Code Snippets (con su propia clave secreta) y agregue `WP_UPLOAD_SECRET` en Vercel con el mismo valor, para poder probar la subida real de punta a punta antes de publicar.
