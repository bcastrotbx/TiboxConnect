...
## Hallazgo real: límite de tamaño de request de `/api/upload-image` — [CORREGIDO]
...
**Solución aplicada en ese momento** (fuera del repo, en el script one-off de carga): las 13 fotos se recomprimieron a JPEG calidad 82 con `sips` antes de subirlas.

**Corregido en el código** (mismo día, ajuste aparte — ver `src/services/portalImageUploadService.js`): `uploadPortalImage()` ahora comprime automáticamente en el navegador (redimensiona a 2000px de lado más largo + recodifica a JPEG calidad 0.82 vía `<canvas>`) cualquier imagen que supere 4MB, antes de intentar subirla — mismo criterio que se usó a mano acá, ahora aplicado a los 6 puntos de entrada que comparten `ImageUploadInner`/`ImageUploadField` (Noticias, Infografías, Portada, banner/logo/galería de eventos) sin que el usuario tenga que hacer nada. `MAX_BYTES` en `api/upload-image.js` se mantiene como estaba — sigue siendo el respaldo por si la compresión no fuera suficiente, pero ya no es el límite real que decide si algo pasa o no.
