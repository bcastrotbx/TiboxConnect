...
**La autenticación de administradores (Fase 5), el contenido esencial del portal (Fase 6-7-8) y los tres formularios públicos ya son reales.** Noticias, infografías, videos/webinars y eventos se leen y se administran contra Supabase de verdad — se agregaron por urgencia de negocio (evento de la primera semana de agosto de 2026), acotados a lo esencial. Contacto, opinión de cliente y el lead de infografías guardan datos reales en Supabase (ver `src/services/formService.js`). Lo que sigue sin conectar: `resources` genéricos y galería de eventos. Antes de asumir que algo "funciona", léase lo siguiente:
...
- **Lo que sigue sin conectar:** `resources` genéricos, galería de eventos realizados como sección general, servicios TIBOX (catálogo) y portada/hero (se lee de Supabase pero no hay UI de edición todavía) — todo esto sigue en `src/data/seed/`, según el caso. Ver el detalle de qué quedó explícitamente fuera de alcance en [FASE-06-07-08-CONTENIDO-REAL.md](docs/phases/FASE-06-07-08-CONTENIDO-REAL.md).
- **Formularios públicos reales:** contacto, opinión de cliente y el lead de infografías (`src/services/formService.js`) hacen `insert` reales contra `contact_messages`, `feedback` e `infographic_leads` en Supabase — dejaron de ser simulados poco después del MVP inicial. El lead de infografías, además, expira a las 24 horas por navegador (ver `src/components/Media.jsx`).
...
En resumen: autenticación de administradores, el contenido esencial (noticias/infografías/videos/eventos) y los tres formularios públicos (contacto, opinión, lead de infografías) ya son reales y funcionales de punta a punta; lo que sigue como prototipo visual es el catálogo de servicios TIBOX, la portada/hero (sin UI de edición) y la galería general de eventos.
